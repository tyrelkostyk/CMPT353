...
app.post('/api/register', async (req, res) => {
	...
	try {
		await asyncQuery(insertQuery, [username, password, displayName]);
...
app.post('/api/login', async (req, res) => {
	...
	try {
		response = await asyncQuery(selectUserQuery, [username, password]);
...
app.post('/api/addChannel', authenticate, async (req, res) => {
	...
	try {
		await asyncQuery(insertQuery, [name, description]);
...
app.post('/api/addMessage/:channelId', authenticate, async (req, res) => {
	...
	try {
		await asyncQuery(insertQuery, [author, channelId, text, 0]);
...
app.get('/api/getMessages/:channelId', authenticate, async (req, res) => {
	...
	try {
		result = await asyncQuery(selectQuery, [channelId]);
		const response = JSON.parse(JSON.stringify(result));
...
app.post('/api/addReply/:messageId', authenticate, async (req, res) => {
	...
	try {
		await asyncQuery(insertQuery, [author, messageId, text, 0]);
...
app.get('/api/getReplies/:messageId', authenticate, async (req, res) => {
	...
	try {
		result = await asyncQuery(selectQuery, [messageId]);
		const response = JSON.parse(JSON.stringify(result));
...
app.put('/api/voteMessage/:messageId/:vote', authenticate,  async (req, res) => {
	...
	try {
		const checkVoteResult = await asyncQuery(checkVoteQuery, [username, messageId]);
		...
		if (checkVoteResult.length) {
			...
			if (checkVoteResult[0].vote == vote) {
				...
				const removeVoteQuery = `DELETE FROM ${voteTableName}
										 WHERE username = ? AND
										 messageId = ?`;
				const removeVoterResult = await asyncQuery(removeVoteQuery, [username, messageId]);
				...
				const updateScoreQuery = `UPDATE ${messageTableName}
										  SET score = score - ?
										  WHERE id = ?`;
				const updateScoreResult = await asyncQuery(updateScoreQuery, [voteValue, messageId]);
			...
			// case 2 - different vote i.e. flip vote
				...
				const updateVoteQuery = `UPDATE ${voteTableName}
										 ...
										 WHERE username = ? AND
			 							 messageId = ?`;
				const updateVoteResult = await asyncQuery(updateVoteQuery, [vote, username, messageId]);
		...
		// no vote exists yet (for this user & message)
			...
			const insertVoteQuery = `INSERT INTO ${voteTableName} (username, messageId, vote)
									 VALUES (?, ?, ?)`;
			const insertVoteResult = await asyncQuery(insertVoteQuery, [username, messageId, vote]);
...
app.get('/api/searchChannels/:search', authenticate, async (req, res) => {
	...
	try {
		result = await asyncQuery(searchQuery, [search]);
		const response = JSON.parse(JSON.stringify(result));
		console.log("Retrieved Channels: ", response);
		res.json(response);
...
app.get('/api/searchMessages/:search', authenticate, async (req, res) => {
	const { search } = req.params;
